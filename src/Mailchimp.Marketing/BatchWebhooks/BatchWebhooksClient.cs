...
public partial class BatchWebhooksClient : IBatchWebhooksClient
{
    private readonly RawClient _client;

    internal BatchWebhooksClient(RawClient client)
    {
        _client = client;
    }
    ...
    /// </summary>
    private WithRawResponseTask<ListBatchWebhooksResponse> ListInternalAsync(
        ListBatchWebhooksRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<ListBatchWebhooksResponse>(
            ListInternalAsyncCore(request, options, cancellationToken)
        );
    }

    private async Task<WithRawResponse<ListBatchWebhooksResponse>> ListInternalAsyncCore(
        ListBatchWebhooksRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                var responseData = JsonUtils.Deserialize<ListBatchWebhooksResponse>(responseBody)!;
                return new WithRawResponse<ListBatchWebhooksResponse>()
                {
                    Data = responseData,
    ...
    private async Task<WithRawResponse<BatchWebhook>> GetAsyncCore(
        ...
        var response = await _client
            .SendRequestAsync(
                new JsonRequest
                {
                    Method = HttpMethod.Get,
                    Path = string.Format(
                        "3.0/batch-webhooks/{0}",
                        ValueConvert.ToPathParameterString(request.BatchWebhookId)
                    ),
                    QueryString = _queryString,
                    Headers = _headers,
                    Options = options,
    ...
    private async Task<WithRawResponse<BatchWebhook>> UpdateAsyncCore(
    ...
    /// </code></example>
    public async Task<Pager<BatchWebhook>> ListAsync(
        ListBatchWebhooksRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        ...
        >
            .CreateInstanceAsync(
                ...
                null,
                response => response.Webhooks?.ToList(),
                null,
                cancellationToken
            )
            .ConfigureAwait(false);
    ...
    /// );
    /// </code></example>
    public WithRawResponseTask<BatchWebhook> GetAsync(
        GetBatchWebhooksRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    ...
    /// );
    /// </code></example>
    public WithRawResponseTask<BatchWebhook> UpdateAsync(
        UpdateBatchWebhooksRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
