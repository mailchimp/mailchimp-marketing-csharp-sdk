...
public partial class RootClient : IRootClient
{
    private readonly RawClient _client;

    internal RootClient(RawClient client)
    {
        _client = client;
    }

    private async Task<WithRawResponse<ListRootResponse>> ListAsyncCore(
    ...
    )
    {
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                var responseData = JsonUtils.Deserialize<ListRootResponse>(responseBody)!;
                return new WithRawResponse<ListRootResponse>()
                {
                    Data = responseData,
    ...
    /// </code></example>
    public WithRawResponseTask<ListRootResponse> ListAsync(
        ListRootRequest request,
        RequestOptions? options = null,
        CancellationToken cancellationToken = default
    )
    {
        return new WithRawResponseTask<ListRootResponse>(
